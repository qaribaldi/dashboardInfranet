...
class InventoryHardwareController extends Controller
{
    ...
    // Kolom standar (tetap dilindungi)
    private array $std = [
        'id_hardware','jenis_hardware','storage_type','tanggal_pembelian','vendor',
        'jumlah_stock','status','tanggal_digunakan','id_pc','created_at','updated_at',
    ];
    ...
    {
        $fromDb = $this->getJenisList();
        $all = array_values(array_unique(array_merge(self::JENIS, $fromDb)));
    ...
    {
        $k = Str::snake(trim($name));
        $k = preg_replace('/[^a-z0-9_]/', '_', strtolower($k));
        $k = preg_replace('/_{2,}/', '_', trim($k, '_'));
        if ($k === '' || preg_match('/^\d/', $k)) $k = 'x_'.$k;
        return $k;
    }

    /** Tambah kolom (opsional admin) */
    ...
    /** Daftar kolom dinamis (non-standar) */
    ...
    /** Klasifikasi tipe untuk form/index */
    private function columnKinds(string $table): array
    {
        $cols = \Schema::getColumnListing($table);
        $dateCols = []; $datetimeCols = []; $boolCols = [];
        ...
        try {
            ...
            foreach ($cols as $c) {
                ...
                // tinyint(1) sering terbaca boolean oleh doctrine
                if (in_array($t, ['boolean','bool'])) $boolCols[] = $c;
            }
        } catch (\Throwable $e) {
            foreach ($cols as $c) {
                if (preg_match('/(^tanggal_|_date$)/', $c)) $dateCols[] = $c;
                if (preg_match('/(_at$|_datetime$|^waktu_)/', $c)) $datetimeCols[] = $c;
                if (preg_match('/(^is_|_flag$|_aktif$|_active$|_bool$)/', $c)) $boolCols[] = $c;
            }
        }

        $strip = ['created_at','updated_at'];
        return [
            'dateCols'     => array_values(array_diff($dateCols, $strip)),
            'datetimeCols' => array_values(array_diff($datetimeCols, $strip)),
            'boolCols'     => array_values(array_diff($boolCols, $strip)),
        ];
    }

    /** Rules dinamis biar kolom ekstra ikut tervalidasi */
    private function extraRules(): array
    {
        $rules = [];
        foreach ($this->extraColumns() as $col) $rules[$col] = 'nullable';
        return $rules;
    }

    /** Ambil payload kolom dinamis dari request (sesuai pola menu lain) */
    private function payloadExtraFromRequest(Request $req): array
    {
        $out = [];
    ...
    public function renameColumn(Request $request)
    {
        ...
        if (in_array($from, $this->std, true)) return back()->with('error','Tidak boleh mengubah kolom standar.');
        ...
        return back()->with('success',"Kolom '$from' diubah menjadi '$to'.");
    }
    ...
    public function dropColumn(Request $request)
    {
        ...
        if (in_array($col, $this->std, true)) return back()->with('error','Tidak boleh menghapus kolom standar.');
        ...
        return back()->with('success',"Kolom '$col' dihapus.");
    }
    ...
    public function index(Request $req)
    {
        ...
        $base = InventoryHardware::query()->with('pcs:id_pc');

        if ($q) {
            $cols = array_values(array_diff(
                \Schema::getColumnListing($this->table), ['created_at','updated_at']
            ));
        ...
        if ($jenis) {
            ...
            if ($jenis === 'storage' && $storageType) $base->where('storage_type', $storageType);
        }

        $items = $base->with(['pcs' => fn($q) => $q->select('asset_pc.id_pc')])
                      ->orderBy('id_hardware')
                      ->paginate(12)->appends($req->query());

        $jenisList = $this->getJenisList();
        $kinds = $this->columnKinds($this->table); // dateCols, datetimeCols, boolCols

        return view('inventory.hardware.index', compact('items','jenisList') + $kinds);
    }
    ...
    public function create()
    {
        ...
        $fields = [];
        foreach ($columns as $col) if (!in_array($col,$skip)) $fields[$col] = ucwords(str_replace('_',' ',$col));
    ...
    public function store(Request $req)
    {
        ...
        if ($data['jenis_hardware'] !== 'storage') $data['storage_type'] = null;

        $extra = $this->payloadExtraFromRequest($req);
        ...
        DB::transaction(function () use ($data, $extra) {
            ...
            $hw = InventoryHardware::create(array_merge([
            ...
            if ($nSelected > 0) {
                ...
                $attach  = [];
                foreach ($selectedPcs as $idPc) $attach[$idPc] = ['tanggal_digunakan' => $tanggal];
    ...
    public function update(Request $req, InventoryHardware $hardware)
    {
        ...
        DB::transaction(function () use ($hardware, $data, $extra) {
            ...
            if ($finalStock < 0) abort(422, "Stok tidak mencukupi. Penambahan alokasi membutuhkan ".count($added)." unit.");

            $hardware->update(array_merge([
                'id_hardware'       => $data['id_hardware'],
                'jenis_hardware'    => $data['jenis_hardware'],
                'tanggal_pembelian' => $data['tanggal_pembelian'] ?? null,
                'vendor'            => $data['vendor'] ?? null,
            ...
            if (!empty($added)) {
                $tanggal = $data['tanggal_digunakan'] ?? null;
                $attach  = [];
                foreach ($added as $idPc) $attach[$idPc] = ['tanggal_digunakan' => $tanggal];
    ...
    /** ===== Import CSV (tanpa perubahan logika utama) ===== */
    public function importForm() { return view('inventory.hardware.import'); }
