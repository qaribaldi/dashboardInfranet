...
@section('content')
  <h2 class="text-2xl font-bold mb-6">{{ $mode === 'create' ? 'Tambah' : 'Edit' }} Aset PC</h2>

  <form action="{{ $mode === 'create' ? route('pc.store') : route('pc.update',$data->id_pc) }}" method="POST" class="space-y-5">
    @csrf
      ...
      @foreach($fields as $name => $label)
        <div>
          ...
          <input id="{{ $name }}" name="{{ $name }}" value="{{ old($name, $data->{$name}) }}"
                 class="w-full rounded-lg border border-gray-300 px-3 py-2" {{ $name==='id_pc' ? ($mode==='edit'?'':'') : '' }}/>
  ...
  <textarea id="catatan_histori" name="catatan_histori" class="w-full rounded-lg border border-gray-300 px-3 py-2" rows="3" placeholder="Misal: Upgrade RAM dari 8GB ke 16GB, ganti SSD, perbaikan PSU, dll.">{{ old('catatan_histori') }}</textarea>
    ...
    <div class="flex items-center gap-3">
      ...
      <a href="{{ route('pc.index') }}" class="rounded-lg border px-4 py-2 hover:bg-gray-50">Batal</a>
    </div>
  </form>
@endsection
