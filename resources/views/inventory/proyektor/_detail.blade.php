{{-- resources/views/inventory/proyektor/_detail.blade.php --}}
@php
    ...
    $allCols = isset($cols) && is_array($cols)
        ? $cols
        : array_values(array_diff(Schema::getColumnListing('asset_proyektor'), ['created_at','updated_at']));
    ...
    // Riwayat (bila tidak dikirim controller, jadikan koleksi kosong)
...
<div class="space-y-6">
  ...
  @unless(request('readonly') == '1')
    <div class="flex justify-end gap-2">
      <a href="{{ route('inventory.proyektor.edit', $data->id_proyektor) }}"
         class="rounded-lg border border-gray-200 px-4 py-2 hover:bg-gray-50">
        Edit
      </a>
      <form action="{{ route('inventory.proyektor.destroy', $data->id_proyektor) }}" method="POST"
