...
@section('content')
  <h2 class="text-2xl font-bold mb-4">Import CSV - Aset PROYEKTOR</h2>

  <div class="mb-4">
    <a href="{{ route('inventory.proyektor.template') }}"
  ...
  <form action="{{ route('inventory.proyektor.importStore') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    @csrf
    ...
    <div>
      ...
      <select name="mode" class="rounded-lg border px-3 py-2">
        <option value="upsert">Upsert (insert baru + update jika id_proyektor sudah ada)</option>
        <option value="insert_only">Insert Only (lewati jika id_proyektor sudah ada)</option>
      </select>
      <p class="text-xs text-gray-500 mt-1">
        <strong>Upsert:</strong> menambah data baru sekaligus mengupdate data lama jika <code>id_proyektor</code> sama.<br>
        <strong>Insert Only:</strong> hanya menambah data baru, baris dengan <code>id_proyektor</code> yang sudah ada akan dilewati.
      </p>
    </div>
    ...
    <div class="pt-2 flex items-center gap-3">
      ...
      <a href="{{ route('inventory.proyektor.index') }}" class="rounded-lg border px-4 py-2 hover:bg-gray-50">Kembali</a>
    </div>
  </form>
@endsection
