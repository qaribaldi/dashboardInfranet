...
@section('content')
  ...
  <div class="bg-white rounded-xl border border-gray-200 p-5">
    @php
      // Ambil fillable; kalau model belum mendefinisikan, fallback ke semua kolom (tanpa timestamps)
      $fillable = (new \App\Models\AssetProyektor)->getFillable();
      if (empty($fillable)) {
        $fillable = array_values(array_diff(\Schema::getColumnListing('asset_proyektor'), ['created_at','updated_at']));
      }

      // Label rapi + helper tampilan
      $labels = [];
  ...
  @php
    // Amankan jika controller belum mengirim $histories
