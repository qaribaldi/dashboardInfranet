{{-- resources/views/inventory/ac/show.blade.php --}}
@extends('layouts.app')

@section('content')
  <div class="flex items-center justify-between mb-6">
    <h2 class="text-2xl font-bold">Detail Aset AC - {{ $data->id_ac }}</h2>
    <div class="flex gap-2">
      <a href="{{ route('inventory.ac.edit',$data->id_ac) }}" class="rounded border px-4 py-2 hover:bg-gray-50">Edit</a>
      <a href="{{ route('inventory.ac.index') }}" class="rounded border px-4 py-2 hover:bg-gray-50">Kembali</a>
    </div>
  </div>
  ...
  <div class="bg-white rounded-xl border border-gray-200 p-5">
    @php
      // Ambil fillable; bila model belum mendefinisikan semua kolom, fallback ke skema tabel
      $fillable = (new \App\Models\AssetAc)->getFillable();
      if (empty($fillable)) {
        $fillable = array_values(array_diff(\Schema::getColumnListing('asset_ac'), ['created_at','updated_at']));
      }

      // Urutkan: kolom standar duluan agar rapi, sisanya menyusul
      $std = [
        'id_ac','unit_kerja','user','jabatan','ruang','tipe_asset','merk',
        'spes','remote','tahun_pembelian',
      ];
      $extra   = array_values(array_diff($fillable, $std));
      $ordered = array_values(array_unique(array_merge($std, $extra)));

      // Label rapi
      $labels = [];
      foreach ($ordered as $f) $labels[$f] = ucwords(str_replace('_',' ', $f));
  ...
  {{-- RIWAYAT PERUBAHAN --}}
  @php
    // Controller show() idealnya mengirim $histories; fallback kosong agar view aman
