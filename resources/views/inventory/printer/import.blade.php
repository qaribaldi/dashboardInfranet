...
@section('content')
  <h2 class="text-2xl font-bold mb-4">Import CSV - Aset PRINTER</h2>

  <div class="mb-4">
    <a href="{{ route('inventory.printer.template') }}"
  ...
  <form action="{{ route('inventory.printer.importStore') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    @csrf
    ...
    <div>
      ...
      <select name="mode" class="rounded-lg border px-3 py-2">
        <option value="upsert">Upsert (insert baru + update jika id_printer sudah ada)</option>
        <option value="insert_only">Insert Only (lewati jika id_printer sudah ada)</option>
      </select>
      <p class="text-xs text-gray-500 mt-1">
        <strong>Upsert:</strong> menambah data baru sekaligus mengupdate data lama jika <code>id_printer</code> sama.<br>
        <strong>Insert Only:</strong> hanya menambah data baru, baris dengan <code>id_printer</code> yang sudah ada akan dilewati.
      </p>
    </div>
    ...
    <div class="pt-2 flex items-center gap-3">
      ...
      <a href="{{ route('inventory.printer.index') }}" class="rounded-lg border px-4 py-2 hover:bg-gray-50">Kembali</a>
    </div>
  </form>
@endsection
