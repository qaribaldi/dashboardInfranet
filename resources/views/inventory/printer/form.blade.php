...
@section('content')
  ...
  <form action="{{ $mode === 'create' ? route('printer.store') : route('printer.update',$data->id_printer) }}" method="POST" class="space-y-5">
    @csrf
  ...
  <textarea id="catatan_histori" name="catatan_histori" class="w-full rounded-lg border border-gray-300 px-3 py-2" rows="3" placeholder="Misal: Upgrade RAM dari 8GB ke 16GB, ganti SSD, perbaikan PSU, dll.">{{ old('catatan_histori') }}</textarea>
    ...
    <div class="flex items-center gap-3">
      ...
      <a href="{{ route('printer.index') }}" class="rounded-lg border px-4 py-2 hover:bg-gray-50">Batal</a>
    </div>
  </form>
@endsection
