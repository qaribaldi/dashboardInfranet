@php
  ...
  $allCols = array_values(array_diff(
      Schema::getColumnListing('inventory_labkom'),
  ...
  $display = function($val) {
      ...
      if ($val instanceof \Carbon\Carbon) return $val->format('Y-m-d H:i');
      return $val;
  };
@endphp
...
<div class="space-y-6">
  ...
  @unless(request('readonly') == '1')
    <div class="flex justify-end gap-2">
      <a href="{{ route('inventory.labkom.edit', $data->id_pc) }}"
         class="rounded-lg border px-4 py-2 hover:bg-gray-50">Edit</a>

      <form action="{{ route('inventory.labkom.destroy', $data->id_pc) }}"
